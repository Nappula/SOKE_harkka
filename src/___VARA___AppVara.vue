    ...
    <div class="weather-icon-wrapper">
                ...
                <forecastTemplate></forecastTemplate>
                    <!--<div class="forecast">
                        ...
                        <div class="forecast-icon"><img :src="'http://openweathermap.org/img/wn/' + fcDay1Icon + '@2x.png'" /></div>
                        <div>{{Math.round(fcDay1Temp)}} °C</div>
                    </div>
    
                    <div class="forecast">
                        <div class="weekday">{{ getWeekday(2) }}</div>
                        <div class="forecast-icon"><img :src="'http://openweathermap.org/img/wn/' + fcDay2Icon + '@2x.png'" /></div>
                        <div>{{Math.round(fcDay2Temp)}} °C</div>
                    </div>
    
                    <div class="forecast">
                        <div class="weekday">{{ getWeekday(3) }}</div>
                        <div class="forecast-icon"><img :src="'http://openweathermap.org/img/wn/' + fcDay3Icon + '@2x.png'" /></div>
                        <div>{{Math.round(fcDay3Temp)}} °C</div>
                    </div>
    
                    <div class="forecast">
                        <div class="weekday">{{ getWeekday(4) }}</div>
                        <div class="forecast-icon"><img :src="'http://openweathermap.org/img/wn/' + fcDay4Icon + '@2x.png'" /></div>
...
<script>
import forecastTemplate from './components/forecastTemplate.vue'
    ...
    methods: {
        ...
        fetchWeather(e) {
            if (e.key == "Enter") { // kun on painettu Enteriä
                ...
                //noudetaan syöte
                fetch(`${this.url_base}weather?q=${this.query}&units=metric&lang=fi&APPID=${this.api_key}`) //browser's fetch apihttps://developers.google.com/web/updates/2015/03/introduction-to-fetch   --  https://blog.logrocket.com/axios-or-fetch-api/
                    .then(res => {
                        return res.json();
